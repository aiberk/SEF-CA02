...
module.exports = {
  index: (req, res) => {
    Prompt.find({ choice: "Rue" }) // filter by choice field
      .then((prompts) => {
        res.render("rue/index", { prompts: prompts });
      })
      .catch((err) => {
      ...
      .then((prompt) => {
        if (
          ...
          User.updateOne(
            ...
            .then(() => {
              res.redirect("/rue"); // Redirect to the prompt index page
            })
            .catch((error) => {
